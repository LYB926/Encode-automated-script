...
def generate(rawName):
    ...
    outputPath = '/data/WispChan/x264_benchmark_output/CQP/'+rawNameSplit[0]+'/'
    ...
    file.write('mkdir /data/WispChan/x264_benchmark_output/CQP/' +
               rawNameSplit[0] + '\n')

    # Write to file
    for qp in qpList:
        cmd = './' + encoderName + ' --preset slow --subme 9 --bframes 8 --me umh --keyint infinite --tune ssim --qp '\
             + qp + ' ' + rawPath + rawName + ' --input-res ' + rawResolution + ' --fps ' \
             + rawFrameRate + ' -o ' + outputPath + \
             rawNameSplit[0] + '_' + qp + '.264 > ' + outputPath + rawNameSplit[0] + '_' + qp + '.log\n'
...
fileNameAll = 'encode_x264_all_cqp.sh'
